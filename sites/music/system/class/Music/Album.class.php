...
class Album
{
	...
	{
		$query = \DB::prepareQuery("SELECT ID FROM Music_Albums WHERE title = %s", $title);
	...
	public static function saveToDB($albums)
	{
		...
		$album_Insert = "INSERT INTO Music_Albums (
			ID,
			image_mediaID,
			timeReleased,
			title
		...
		$albumTracks_Insert = "INSERT INTO Music_AlbumTracks (
			albumID,
			trackID,
			trackNo
		) VALUES";
		...
		$albumTracks_DeleteQuery = \DB::prepareQuery("DELETE FROM Music_AlbumTracks WHERE albumID IN %a", $albumTracks_DeleteIDs);
