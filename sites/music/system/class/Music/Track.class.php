...
class Track
{
	...
	public static function loadByTrack(self $Track)
	{
		...
		$query = \DB::prepareQuery("SELECT
			...
			FROM
				...
				JOIN Media m ON m.ID = t.audio_mediaID
			WHERE
				m.fileHash = %s
				AND t.title = %s
	...
	public static function loadFromDB($trackIDs)
	{
		...
		$audios = \Manager\Media::loadFromDB($audio_mediaIDs);
		...
		$query = \DB::prepareQuery("SELECT trackID, artistID FROM Music_TrackArtists WHERE trackID IN %a", $trackIDs);
		$Result = \DB::queryAndFetchResult($query);

		$artistIDs = array();

		while($trackArtists = $Result->fetch_assoc())
		{
	...
	public static function saveToDB($tracks)
	{
		...
		$track_Insert = "INSERT INTO Music_Tracks (
			ID,
			audio_mediaID,
			image_mediaID,
			timeReleased,
		...
		$trackArtists_Insert = "INSERT INTO Music_TrackArtists (
			trackID,
			artistID
		) VALUES";
		...
		$trackArtists_DeleteQuery = \DB::prepareQuery("DELETE FROM Music_TrackArtists WHERE trackID IN %a", $trackArtists_DeleteIDs);
	...
	public function setAudio(\Media\Audio $Audio)
	{
		$this->Audio = $Audio;
		return $this;
	}
